...
- **CI:** O workflow corre **Chromium, Firefox e WebKit** em paralelo (três jobs); o tempo total depende do GitHub Actions e da rede até ao blog. Também corre **três vezes por dia** (**08:00, 12:00 e 18:00** em Brasília) via agendamento no GitHub Actions.
...
Dispara em `push` e `pull_request` para `main`/`master`, execução manual (`workflow_dispatch`) e **três crons diários** (`11:00`, `15:00` e `21:00` UTC ≈ **08:00, 12:00 e 18:00** em Brasília).  
